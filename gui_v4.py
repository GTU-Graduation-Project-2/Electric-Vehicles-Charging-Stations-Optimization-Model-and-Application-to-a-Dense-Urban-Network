...
class ChargingStationOptimizer:
    ...
    def on_map_click(self, coords):
        ...
        self.station_candidates.append({'lat': lat, 'lon': lon, 'poi': poi})

        idx = len(self.station_candidates)
        color = POI_COLOR[poi]
        num   = POI_TYPE_NUM[poi]
        m = self.map_widget.set_marker(
            lat, lon,
            text=f"{num}-{idx}",
        ...
        m.set_text(f"Type: {poi}\nFixed Cost: {POI_FIXED_COST[poi]} k€")
        self.status_var.set(f"Added station candidate {poi} ({lat:.4f},{lon:.4f})")
    ...
    def _update_markers(self):
        ...
        for idx, c in enumerate(self.station_candidates, start=1):
            ...
            m.set_text(f"Type: {c['poi']}\nFixed Cost: {POI_FIXED_COST[c['poi']]} k€")

        # 3) Seçilen evler
        for sh in self.selected_homes:
            ...
            m.set_text(f"Brand: {v.brand}\nBattery: {v.battery_capacity} kWh\nCharge rate: {v.charge_rate} kW")

        # 4) Seçilen istasyonlar
        for s in self.selected_stations:
            ...
            m.set_text(f"Fixed Cost: {POI_FIXED_COST[s['poi']]} k€\nCapacity: {self.capacity_var.get()} kWh/day")
    ...
    def _solve_model(self, max_st, rnv, evr, capacity, radius):
        ...
        self.selected_homes = [{'home':h, 'vehicle': random.choice([Renault, Ford, Tesla, Nissan])()} for h in sampled]
        ...
        for i in I:
            ...
            for j in I:
                if i<j:
                    h1 = self.selected_homes[i]['home']; h2 = self.selected_homes[j]['home']
        ...
        for j in J:
                m.add_constraint(m.sum(D[i] * y[i,j] for i in I) <= capacity * x[j])
        for j in J:
        ...
        self.selected_stations = [
            {'lat':pt['lat'],'lon':pt['lon'],'poi':pt['poi'],'type':pt['poi']}
            for j,pt in enumerate(self.station_candidates) if x[j].solution_value>0.5
        ]
    ...
    def open_results_window(self):
        ...
        for i, s in enumerate(self.selected_stations,1):
            tree.insert('', 'end', values=(i, s['poi'], f"{s['lat']:.4f}", f"{s['lon']:.4f}"))
        tree.pack(fill=X)
        ...
        for lbl,var in [("Cost (k€)",self.cost_var),("Semi-fast CS",self.semi_var),
                        ("Fast CS",self.fast_var),("Chargers",self.chargers_var),
        ...
        self.ax.clear()
        x = [0, self.ev_rate_var.get()]; y = [0, self.solution_obj]
        self.ax.plot(x, y, 'o-'); self.ax.set_xlabel('EV Penetration (%)')
        self.ax.set_ylabel('Cost (k€)'); self.ax.grid(True, linestyle='--', alpha=0.6)

...
class ChargingStationOptimizer:
    ...
    def ensure_selected_homes(self, evr):
        """self.selected_homes boşsa EV‐penetrasyon oranına göre örnekler."""
        if self.selected_homes:        # zaten doluysa dokunma
            return
    ...
    def run_optimization(self):
        ...
        evr = self.ev_rate_var.get()   # önce lazım olacak
        self.ensure_selected_homes(evr)      # ① EV/araç seç
        self.generate_daily_trips()          # ② Yolculuk zincirleri oluştur
        ...
        for i in I:
            ...
            for j in I:
                if i < j:
                    ...
                    dist = road_distance_km(h1['lat'], h1['lon'], h2['lat'], h2['lon'])

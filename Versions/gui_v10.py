...
class ChargingStationOptimizer:
    ...
    def show_legend(self):
        # ---- ikon tanımları ----
        legend_items = [
            ("green",    "Yeşil  : Ev / Araç konumu"),
            ("#ffc107",  "Sarı   : Home-type istasyon adayı"),
            ("#fd7e14",  "Turuncu: Parking-type istasyon adayı"),
            ("#007bff",  "Mavi   : Fuel-type (hızlı) adayı"),
            ("#17a2b8",  "Turkuaz: Seçili EV"),
            ("#6f42c1",  "Mor    : Seçili istasyon"),
        ]
    ...
    def run_optimization(self):
        ...
        for rec in self.trip_log:
            print(rec)
        print("-"*40, f"{len(self.trip_log)} trip kaydedildi\n")
        
        print(f"[Trip-based demand] {len(self.trip_log)} tripten "
            f"toplam {sum(r['cons_kwh'] for r in self.trip_log):.2f} kWh")
    ...
    def build_heatmap(self):
        ...
        if not hasattr(self, "edge_freq") or not self.edge_freq:
            messagebox.showinfo("Info", "Önce trip üretmek için optimizasyonu çalıştırın.")
            return
        ...
        self.status_var.set("Heat-map çizildi (yeşil → kırmızı)")
    ...
    def debug_od(self, selected_homes, station_candidates, d_mat, export_csv=False):
            ...
            if export_csv:
                ...
                print(f"... ayrıntılı tablo '{fn}' dosyasına yazıldı.")
    ...
    def _solve_model(self, max_st, evr, capacity, radius):
        ...
        print(f"[Trip-based demand] {len(self.trip_log)} tripten "
            f"toplam {sum(D):.2f} kWh")                 # DEBUG satırı
        # ====================================================================
